	...
	<div class="col-sm-12">
		<form action="<?=DOMAIN?>?c=<?=$controller?>&action=update&id=<?=$info->id?>" method="post">
			...
			<div class="form-group">
				...
				<input type="number" name="ecr-deducir" id="ecr-deducir" class="form-control" min="0" required="" value="<?=$info->deducir?>">
			</div>

			<div class="form-group">
				<label for="ecr-aumentar">Aumentar:</label>
				<input type="number" name="ecr-aumentar" id="ecr-aumentar" class="form-control" min="0" required="" value="<?=$info->aumentar?>">
			</div>
