...
@section('content')
	...
	<div class="portlet">
		...
		<div class="portlet-body">
			<form method="post" enctype="multipart/form-data" action="{{ route('store-prestamos') }}">
				@csrf
						...
						<div class="">
							<select name="socio" id="socio" class="form-control" required>
								...
								@foreach($socios as $key)
									<option value="{{$key->id}}">{{$key->nombre}} | Cedula: {{$key->cedula}}</option>
								@endforeach
							</select>
						...
						<div class="">
							<select name="tasa_interes" id="tasa_interes" class="form-control" required>
								...
								@foreach([10,12,18,24,30] as $key)
									<option value="{{$key}}" >{{$key}}% anual</option>
								@endforeach
							</select>
						...
						<div class="">
							<select name="tiempo" id="tiempo" class="form-control" required>
								...
								@foreach([1,2,3,4,5] as $key)
									<option value="{{$key}}" >{{$key}} años</option>
								@endforeach
							</select>
